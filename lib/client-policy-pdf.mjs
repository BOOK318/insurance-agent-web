...
function findClientPhone(text) {
  const insuredLabeled = normalizePhone(readLabelValue(text, [
    'Insured Mobile',
    'Insured Phone',
    'Life Insured Mobile',
    'Life Insured Phone',
  ...
  if (insuredLabeled) return insuredLabeled;

  const labeled = normalizePhone(readNonAgentLabelValue(text, CLIENT_PHONE_LABELS));
  if (labeled) return labeled;

  for (const line of normalizePolicyPdfText(text).split('\n')) {
    if (isAgentContext(line)) continue;
...
export function extractClientAndPolicyFromPolicyText(text) {
  ...
  const clientName = insuredName || policyholderName;
  ...
  if (policyholderName) policy.policyholder_name = policyholderName;
  if (insuredName) policy.insured_name = insuredName;
